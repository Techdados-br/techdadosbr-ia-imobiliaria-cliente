...
def cores_tema():
    ...
    if tema == "Escuro":
        return {
            "bg": "#0F172A", "card": "#111827", "card2": "#162033",
            "text": "#F8FAFC", "muted": "#CBD5E1", "border": "#334155",
            "shadow": "0 12px 28px rgba(0,0,0,.35)", "hero1": "#132B57",
            "hero2": "#0F3A4A", "plot_bg": "#0F172A", "plot_grid": "#263244",
            "plot_font": "#F8FAFC",
        }
    return {
        "bg": "#F3F6FA", "card": "#FFFFFF", "card2": "#F8FAFC",
        "text": "#0F172A", "muted": "#475569", "border": "#CBD5E1",
        "shadow": "0 12px 28px rgba(15,23,42,.12)", "hero1": "#DBEAFE",
        "hero2": "#CFFAFE", "plot_bg": "#F3F6FA", "plot_grid": "#D8E0EA",
        "plot_font": "#0F172A",
    }
...
def aplicar_css():
    ...
    <style>
        ...
        section[data-testid="stSidebar"] {{ background: {c['card2']}; border-right: 1px solid {c['border']}; }}
        .td-section {{ margin-top: 30px; margin-bottom: 12px; }}
        .td-section h3 {{ color: {c['text']}; margin-bottom: 4px; font-size: 23px; font-weight: 800; }}
        .td-section p {{ color: {c['muted']}; margin-top: 0; font-size: 13px; }}
        .td-card {{
            background: {c['card']}; border: 1px solid {c['border']}; border-left: 6px solid var(--accent);
            border-radius: 16px; padding: 18px 18px; min-height: 118px; box-shadow: {c['shadow']};
        }}
        .td-card-label {{ color: {c['muted']}; font-size: 12px; font-weight: 800; text-transform: uppercase; letter-spacing: .04em; margin-bottom: 12px; }}
        .td-card-value {{ color: var(--value-color, {c['text']}); font-size: 27px; font-weight: 900; line-height: 1.15; margin-bottom: 8px; }}
        .td-card-caption {{ color: {c['muted']}; font-size: 12px; line-height: 1.35; }}
        .td-alert {{
            background: {c['card']}; border: 1px solid {c['border']}; border-left: 6px solid var(--accent);
            border-radius: 16px; padding: 18px 18px; box-shadow: {c['shadow']}; margin-bottom: 14px;
        }}
        .td-alert-title {{ color: {c['text']}; font-size: 14px; font-weight: 800; margin-bottom: 6px; }}
        .td-alert-text {{ color: {c['text']}; font-size: 13px; line-height: 1.45; }}
        div[data-testid="stMetric"] {{ background: {c['card']}; border: 1px solid {c['border']}; border-radius: 16px; padding: 14px 16px; box-shadow: {c['shadow']}; }}
        hr {{ margin: 28px 0 !important; border-color: {c['border']} !important; }}
    </style>
...
def exibir_executivo(
    ...
    section("Dashboard executivo", "Visão geral dos principais indicadores da carteira imobiliária.")
    ...
    with c3:
        kpi_card("Receita perdida", moeda_br(receita_perdida), "Potencial a recuperar", VERMELHO, VERMELHO)
    with c4:
        cor_inad = VERMELHO if perc_inadimplencia >= 50 else LARANJA
        kpi_card("Inadimplência", percentual_br(perc_inadimplencia, 1), "Percentual sobre a receita", cor_inad, cor_inad)
    with c5:
        cor_ef = VERDE if eficiencia >= 80 else AZUL
        kpi_card("Eficiência", percentual_br(eficiencia, 1), "Ocupação da carteira", cor_ef, cor_ef)

    st.divider()
    section("Resumo executivo", "Leitura rápida do score e dos pontos que exigem atenção.")

    r1, r2, r3 = st.columns(3)
    with r1:
        kpi_card("Score geral", f"{score}/100", "Pontuação consolidada", cor_classificacao, cor_classificacao)
    with r2:
        kpi_card("Classificação", texto_classificacao, "Resultado executivo", cor_classificacao, cor_classificacao)
    with r3:
        cor_vac = LARANJA if vacancia >= 15 else VERDE
        kpi_card("Vacância", percentual_br(vacancia, 1), "Imóveis vagos", cor_vac, cor_vac)

    st.divider()
    section("Indicadores estratégicos", "Receita e inadimplência por bairro para análise de concentração.")

    g1, g2 = st.columns(2)
    with g1:
        fig_receita = grafico_receita_bairro(receita_bairro)
        fig_receita.update_traces(marker_color=AZUL)
        st.plotly_chart(estilizar_figura(fig_receita), use_container_width=True)
    with g2:
        fig_inad = grafico_inadimplencia_bairro(df_inadimplencia)
        fig_inad.update_traces(marker_color=VERMELHO)
        st.plotly_chart(estilizar_figura(fig_inad), use_container_width=True)

    st.divider()
    section("Ocupação dos imóveis", "Distribuição entre imóveis ocupados e vagos.")

    c1, c2 = st.columns([2, 1])
    with c1:
        fig_status = grafico_status_imoveis(total_ocupados, total_vagos)
        st.plotly_chart(estilizar_figura(fig_status), use_container_width=True)
    with c2:
        m1, m2 = st.columns(2)
        with m1:
    ...
    st.divider()
    section("Diagnóstico inteligente", "Principais pontos identificados na análise.")
    ...
    st.divider()
    section("Insights estratégicos", "Oportunidades e observações para apoio à decisão.")

    for insight in insights:
        alerta_card("Insight", str(insight), VERDE)

    st.divider()
    section("Relatório executivo", "Gere o PDF com os principais indicadores, gráficos e parecer executivo.")
