...
def exibir_contratos_prioritarios(df_contratos):
    ...
    tabela_estilizada = (
        ...
        .map(cor_prioridade, subset=["Prioridade"])
        .map(cor_prazo, subset=["Prazo"])
        .map(destaque_valor, subset=["Valor mensal"])
        .set_properties(
            **{
                "color": "#0F172A",
                "border-color": "#E2E8F0",
...
def _figura_receita_bairros_compacta(receita_bairro):
    """Monta um ranking horizontal limpo, sem valores cortados."""
    try:
        ...
        def _rotulo_compacto(valor):
            valor = float(valor)
            if abs(valor) >= 1_000_000:
                return f"R$ {valor / 1_000_000:.1f} mi".replace(".", ",")
            if abs(valor) >= 1_000:
                return f"R$ {valor / 1_000:.1f} mil".replace(".", ",")
            return _moeda_br_contrato(valor)

        rotulos = dados["Receita"].map(_rotulo_compacto)
        cor_grade = "#334155" if tema_visual == "Escuro" else "#E2E8F0"
        cor_eixo = "#CBD5E1" if tema_visual == "Escuro" else "#475569"
        ...
        figura = go.Figure(
            go.Bar(
                ...
                marker=dict(
                    color="#2563EB",
                    line=dict(color="#1D4ED8", width=0.7),
                ),
                customdata=dados["Receita"].map(_moeda_br_contrato),
                hovertemplate=(
                    "<b>%{y}</b><br>"
                    "Receita potencial: %{customdata}"
                    "<extra></extra>"
                ),
                cliponaxis=True,
            )
        ...
        valor_maximo = float(dados["Receita"].max())

        figura.update_layout(
            title=dict(
                text="Receita por bairro — Top 8",
                x=0.0,
                xanchor="left",
                font=dict(size=16),
            ),
            ...
            yaxis_title="",
            margin=dict(l=20, r=24, t=58, b=42),
            uniformtext=dict(minsize=10, mode="hide"),
        )

        figura.update_xaxes(
            range=[0, valor_maximo * 1.04],
            showgrid=True,
            gridcolor=cor_grade,
            zeroline=False,
            tickfont=dict(color=cor_eixo),
            title_font=dict(color=cor_eixo),
            tickformat="~s",
        )

        figura.update_yaxes(
            showgrid=False,
            tickfont=dict(color=cor_eixo, size=12),
            automargin=True,
        )
